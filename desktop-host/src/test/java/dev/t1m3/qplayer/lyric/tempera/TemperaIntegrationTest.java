...
public class TemperaIntegrationTest {
    ...
    public void independentPageOpensFromTopBarAndHidesControlsAfterCompositing() throws Exception {
        verifyIndependentPage(SettingsCatalog.DESKTOP);
        verifyIndependentPage(SettingsCatalog.ANDROID);
    }

    private void verifyIndependentPage(String platform) throws Exception {
        ...
        try {
            ...
            try {
                ...
                player.setTemperaOpen(true);
                queue.flush();
                // Both shells call this shared compositor, including after losing
                // their GPU context in the background. Resources must be reusable.
                dev.t1m3.qplayer.lyric.skia.LyricCompositor shared =
                        new dev.t1m3.qplayer.lyric.skia.LyricCompositor();
                try (Surface surface = Surface.makeRasterN32Premul(800, 720)) {
                    shared.composite(surface.getCanvas(), view.renderer(), view, player, settings,
                            null, 1f, 800, 720);
                    assertTrue(shared.temperaVisible(player));
                    shared.invalidateGpuContext();
                    shared.onRenderResumed();
                    shared.composite(surface.getCanvas(), view.renderer(), view, player, settings,
                            null, 1f, 800, 720);
                    assertTrue(chrome.visible.peek());
                    assertFalse(player.lyricsOpen.peek());
