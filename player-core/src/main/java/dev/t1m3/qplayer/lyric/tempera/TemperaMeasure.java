...
public final class TemperaMeasure {
    ...
    /**
     * 数字字重（folia 的 {@code fontWeight}）映射到 qplayer 的六档字体。
     * 600 是 folia 的主题默认，这里落到粗体，让凝彩的粗野语汇不至于太轻。
     */
    public static Fonts.Weight weightOf(int weight) {
        ...
        if (weight <= 550) return Fonts.Weight.MEDIUM;
        if (weight <= 800) return Fonts.Weight.BOLD;
        return Fonts.Weight.HEAVY;
    }

    /** 按字重取一个尺寸匹配的字体（CJK 由 {@link #fontFor} 再回退一层）。 */
    public static Font font(int weight, float size) {
        return Fonts.get(weightOf(weight), size);
    }
