...
public final class TemperaCinemaCompositions {
    ...
    // 注意：player-core 必须走 Java 8（release 8），所以这里不能用 record，改写为 final 数据类。
    private static final class Win {
        final float x;
        final float y;
        final float width;
        final float height;
        Win(float x, float y, float width, float height) {
            this.x = x;
            this.y = y;
            this.width = width;
            this.height = height;
    ...
    private static final class Bar {
        ...
        final float enterDX;
        final float enterDY;
        Bar(float[] polygon, float enterDX, float enterDY) {
            this.polygon = polygon;
            this.enterDX = enterDX;
            this.enterDY = enterDY;
        }
    }
