...
public final class TemperaTextView {
    ...
    private static final class LineKey {
        final Font font;
        final String text;
        LineKey(Font font, String text) { this.font = font; this.text = text; }
        @Override public boolean equals(Object other) {
            if (!(other instanceof LineKey)) return false;
            LineKey key = (LineKey) other;
            return font == key.font && text.equals(key.text);
        }
        @Override public int hashCode() { return 31 * System.identityHashCode(font) + text.hashCode(); }
    }
    ...
    static void releaseFont(Font font) {
        Iterator<java.util.Map.Entry<LineKey, TextLine>> entries = LINES.entrySet().iterator();
        while (entries.hasNext()) {
            java.util.Map.Entry<LineKey, TextLine> entry = entries.next();
