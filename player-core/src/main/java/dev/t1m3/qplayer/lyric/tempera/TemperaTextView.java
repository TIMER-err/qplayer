...
public final class TemperaTextView {
    ...
    private static final IdentityHashMap<Font, LinkedHashMap<String, TextLine>> LINES =
            new IdentityHashMap<>();
    /**
    ...
    private static TextLine lineFor(Font font, String text) {
        LinkedHashMap<String, TextLine> bucket = LINES.get(font);
        if (bucket == null) {
            bucket = new LinkedHashMap<>();
            LINES.put(font, bucket);
        }
        TextLine cached = bucket.get(text);
        if (cached != null) return cached;
        TextLine line = TextLine.make(text, font);
        if (bucket.size() >= LINE_CACHE_LIMIT) {
            Iterator<Map.Entry<String, TextLine>> it = bucket.entrySet().iterator();
            if (it.hasNext()) {
                TextLine oldest = it.next().getValue();
                it.remove();
                if (oldest != null) oldest.close();
            }
        }
        bucket.put(text, line);
        return line;
    }

    /** 释放全部缓存的原生文本行（渲染线程销毁时调用）。之后仍可继续使用本类，缓存会重建。 */
    public static void dispose() {
        for (LinkedHashMap<String, TextLine> bucket : LINES.values()) {
            for (TextLine line : bucket.values()) {
    ...
    public static void paint(Canvas canvas, List<View> views, double time, float motionAmount,
                             ...
                             String paper, float rippleAmount) {
        ...
        // 4) 落字涟漪：每个字落位的瞬间从字心扩散一圈细环，快速淡出。这是「歌词本身」的
        //    特效而不是背景装饰——字一个个砸下来时，环一个个荡开，节奏感直接可见。
        if (rippleAmount > 0.001f) {
            drawRipples(canvas, views, time, motionAmount, rippleAmount, tone);
        }
    }

    /** 落字涟漪的存活时长（秒）。 */
    private static final double RIPPLE_LIFE = 0.75;

    private static void drawRipples(Canvas canvas, List<View> views, double time,
                                    float motionAmount, float rippleAmount, String tone) {
        Paint p = paint();
        for (View view : views) {
            double dt = time - view.motion.settleTime;
            if (dt < 0 || dt > RIPPLE_LIFE) continue;
            TemperaMotion.Frame frame = TemperaMotion.resolve(view.motion, time, motionAmount);
            if (!frame.visible) continue;
            float progress = (float) (dt / RIPPLE_LIFE);
            float x = view.baseX + frame.x;
            float y = view.baseY + frame.y;
            float radius = view.fontSize * (0.30f + 0.70f * progress);
            float alpha = (float) Math.pow(1f - progress, 1.8) * 0.5f
                    * rippleAmount * frame.alpha;
            if (alpha <= 0.004f || radius <= 0.5f) continue;
            float stroke = Math.max(0.7f, view.fontSize * 0.04f * (1f - progress));
            p.setShader(null);
            p.setBlendMode(BlendMode.SRC_OVER);
            p.setColor(TemperaColor.withAlpha(
                    tone != null ? tone : view.displayColor, 1f));
            p.setAlphaf(alpha);
            p.setStroke(true);
            p.setStrokeWidth(stroke);
            canvas.drawCircle(x, y, radius, p);
        }
        // 立刻还原成填充模式：这个画笔是共享的，下一处绘制可能直接拿去填字。
        p.setStroke(false);
    }
